...
@app.get("/users/nocache/{user_id}")
def read_user_no_cache(user_id: int, db: Session = Depends(get_db)):
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    return user

@app.get("/users/withcache/{user_id}")
def read_user_with_cache(user_id: int, db: Session = Depends(get_db)):
